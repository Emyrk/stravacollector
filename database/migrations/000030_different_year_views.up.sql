		...
		FROM
			(
				...
				FROM
					segment_efforts
				WHERE
					segment_id = any(ARRAY(SELECT segments FROM competitive_routes WHERE name = 'das-hugel-2023'))
				ORDER BY
...
WHERE
	segment_ids @> ARRAY(SELECT segments FROM competitive_routes WHERE name = 'das-hugel-2023')
;
--
--
CREATE OR REPLACE VIEW athlete_hugel_count_2023 AS
SELECT
	athlete_id, count(*) AS count
FROM
	athletes
		INNER JOIN
	hugel_activities_2023
	ON athletes.id = hugel_activities_2023.athlete_id
GROUP BY athlete_id;
