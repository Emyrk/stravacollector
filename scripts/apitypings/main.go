...
func main() {
	...
	for _, dir := range externalTypePkgs {
		...
		if err != nil {
			log.Fatal().
				Err(err).
				Str("dir", dir).
				Msg("failed to generate typescript types")
		}
		extGen.onlyOptIn = true
		external = append(external, extGen)
	}

	_, _ = fmt.Print("// Code generated by 'make site/strava-frontend/src/api/typesGenerated.ts'. DO NOT EDIT.\n\n")
	for _, baseDir := range baseDirs {
		_, _ = fmt.Printf("// The code below is generated from %s.\n\n", strings.TrimPrefix(baseDir, "./"))
		output, err := Generate(log, baseDir, external...)
		if err != nil {
			log.Fatal().Err(err).Msg("generate")
		}
	...
	for i, ext := range external {
		...
		for {
			...
			if err != nil {
				log.Fatal().Err(err).Msg("generate external")
			}
...
func Generate(log zerolog.Logger, directory string, externals ...*Generator) (string, error) {
	ctx := context.Background()
...
func ParseDirectory(ctx context.Context, log zerolog.Logger, directory string, externals ...*Generator) (*Generator, error) {
	g := &Generator{
		log:       log,
...
func GenerateFromDirectory(ctx context.Context, log zerolog.Logger, directory string, externals ...*Generator) (*Generator, error) {
...
func (g *Generator) typescriptType(ty types.Type) (TypescriptType, error) {
	...
	case *types.Named:
		...
		case "github.com/Emyrk/strava/api/modelsdk.StringInt":
			return TypescriptType{ValueType: "string"}, nil
		case "github.com/Emyrk/strava/api/modelsdk/sdktype.StringInt":
			return TypescriptType{ValueType: "string"}, nil
		case "net/url.URL":
			return TypescriptType{ValueType: "string"}, nil
		case "time.Time":
		...
		if obj != nil {
			...
			if !local {
				...
				g.log.Debug().
					Str("name", objName).
					Str("ext_pkg", objGen.pkg.String()).
					Msg("found external type")
			}
