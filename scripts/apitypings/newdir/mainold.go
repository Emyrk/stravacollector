...
const (
	baseDir = "./api/modelsdk"
	indent  = "  "
)

func main() {
	log := zerolog.New(os.Stderr).With().Timestamp().Logger()
	output, err := Generate(log, baseDir)
	if err != nil {
		log.Fatal().Err(err).Msg("failed to generate typescript types")
	}
...
func Generate(logger zerolog.Logger, directory string) (string, error) {
	ctx := context.Background()
	codeBlocks, err := GenerateFromDirectory(ctx, logger, directory)
	if err != nil {
		return "", err
	}
...
func GenerateFromDirectory(ctx context.Context, logger zerolog.Logger, directory string) (*TypescriptTypes, error) {
	g := Generator{
		log:      logger,
...
func (g *Generator) typescriptType(ty types.Type) (TypescriptType, error) {
	...
	case *types.Interface:
		...
		//    }
		//  }
		return TypescriptType{}, errors.New("only empty interface types are supported")
	case *types.TypeParam:
		...
		if !ok {
			...
			return TypescriptType{}, errors.New("type param must be an interface")
		}
